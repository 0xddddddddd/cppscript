...
namespace cpps
{
	...
	cpps::cpps_value cpps_mysql::call(std::string spname, cpps::cpps_value params)
	{
		...
		int32 param_count = (int32)paramlist.size();
		if (mysql_stmt_attr_set(mysql_stmt, STMT_ATTR_PREBIND_PARAMS, &param_count))
		{
		...
		//执行语句
		int32 code = mariadb_stmt_execute_direct(mysql_stmt, proc.c_str(), -1);
		if (code != 0)
		{
			seterror(mysql_stmt_error(mysql_stmt));
			return nil;
	...
	{
		printf("Error(%d) [%s] \"%s\"", mysql_stmt_errno(mysql_stmt),
